...
function seedDemoUser() {
  ...
  if (!users['demo@crypto.com']) {
    users['demo@crypto.com'] = {
      name: 'Demo User',
  ...
  try {
    ...
    if (state.allCoins.length) {
      ...
      document.getElementById('coins-loading').innerHTML =
        '<div class="empty-state"><div class="empty-icon">⚠️</div><h3>Unable to fetch data</h3><p>Please check your internet connection.</p></div>';
    }
  }
  showLoading(false);
}
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="action-btns">
          ...
          <button class="btn-sm" onclick="openCoinModal('${coin.id}')">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="m19 9-5 5-4-4-3 3"></path></svg>
            Chart
          </button>
        </div>
      </td>
...
async function loadCoinChart(days) {
  ...
  try {
    ...
    state.coinDetailChart = chart; // save ref

    const candlestickSeries = chart.addCandlestickSeries({
      upColor: '#22d3a0', downColor: '#ff5b6e', borderVisible: false,
      wickUpColor: '#22d3a0', wickDownColor: '#ff5b6e'
    });
    ...
    container.innerHTML = '<div style="display:flex;height:100%;align-items:center;justify-content:center;color:var(--loss);">Failed to load chart data. 1 call per min limit maybe reached.</div>';
